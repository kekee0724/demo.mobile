...
export namespace myFollowModel {

  export const namespace = Namespaces.myFollow;
  ...
  export const reducers: ReducersMapObject<any, AnyAction> = {
    ...CoreReducers,
    getCustomerPage(state: any, { datas, thisChangeId, inputID }) {
      if (thisChangeId < changeCounter) return state;
      let data = {};
      data[`datas${inputID}`] = { ...datas, refreshing: false, hasMore: datas.currentPage < datas.totalPages };
  ...
  export const effects: EffectsMapObject = {
    ...
    *initPage({ params, message }, { put }) {
      ...
      try {
        ...
        yield put({ type: `getUserInfo`, userID: params.inputerId, message });
      } catch (e) {
    ...
    *getUserFollow({ params, message }, { call, put }) {
      try {
        ...
        let datas;
        datas = yield call(followService.getMyfollow, { ...params });
        yield put({ type: "getCustomerPage", datas, thisChangeId, inputID: params.inputerId });
      } catch (e) {
         console.log('catcherror', e?.errmsg||e);
         message!.error(`${e?.errmsg || e}`)
      } finally {
