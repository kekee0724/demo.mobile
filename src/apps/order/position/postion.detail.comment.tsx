...
import { Namespaces, positiondetailcommentModel } from "@reco-m/order-models";
import { noPassByName, CommentAuditStatusEnum } from "@reco-m/ipark-common";
import { getCommentAuditStatus } from "@reco-m/order-common";
export namespace PositionDetailComment {
    export interface IProps<S extends IState = IState> extends ViewComponent.IProps<S> {
        id: any;
        title: any;
    }

    export interface IState extends ViewComponent.IState, positiondetailcommentModel.StateType {}

    export class Component<P extends IProps = IProps, S extends IState = IState> extends ViewComponent.Base<P, S> {
        namespace = Namespaces.positiondetailcomment;
        ...
        loadAll() {
            ...
            this.dispatch({
                ...
                params: { id: this.props.id, pageIndex: state!.pageIndex + 1, pageSize: 5 },
            });
        }
                ...
                <div className="back">
                    <WingBlank>
                        ...
                        <Button
                            ...
                            onClick={() => this.goTo(`positionCommentList?title=${this.props.title && this.props.title}&detailId=${this.props.id}`)}
                        >
                            更多
                        </Button>
                        <WhiteSpace />
        ...
        render(): React.ReactNode {
            return (
                <List renderHeader={<div>用户评价</div>}>
                    <>
                        {this.renderResourceDetailComment() || null}
                        {this.renderLoadMore()}
                    </>
                </List>
            );
    ...
    export const Page = template(Component, (state) => state[Namespaces.positiondetailcomment]);
}
